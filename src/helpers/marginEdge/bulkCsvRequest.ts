...
import { processBeverageData } from "./processBeverageData.ts";
...
export const bulkCsvRequest = async (csvFile: File, markupMultiplier: number | string, costPercentage: number | string, ozPerPour: number | string, slidingScale: SlidingScale) => {
  ...
  const processedData = processBeverageData(data, Number(markupMultiplier), Number(costPercentage), Number(ozPerPour), slidingScale);
